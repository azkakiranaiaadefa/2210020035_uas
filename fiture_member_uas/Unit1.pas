...
begin
  try
    ...
    ZCustomer.SQL.Clear;
    ZCustomer.SQL.Add('INSERT INTO kustomer (NIK, NAMA, TELP, EMAIL, ALAMAT) ' +
                       'VALUES (:NIK, :NAMA, :TELP, :EMAIL, :ALAMAT)');
...
begin
ShowMessage('Anda memilih: ' + DBComboBox1.Text);
end;

procedure TForm1.FormCreate(Sender: TObject);
begin
  DBComboBox1.Items.Add('Member');
  DBComboBox1.Items.Add('Non-Member');
end;
...
procedure TForm1.Button6Click(Sender: TObject);
 ReportText := TStringList.Create;
  try
    ...
    ZCustomer.Close;
    ZCustomer.SQL.Text := 'SELECT * FROM kustomer';
    ZCustomer.Open;

    // Iterate through the dataset and append data to ReportText
    while not ZCustomer.Eof do
    begin
      ReportText.Add('NIK: ' + ZCustomer.FieldByName('nik').AsString);
      ReportText.Add('NAMA: ' + ZCustomer.FieldByName('nama').AsString);
      ReportText.Add('TELP: ' + ZCustomer.FieldByName('telp').AsString);
      ReportText.Add('EMAIL: ' + ZCustomer.FieldByName('email').AsString);
      ReportText.Add('ALAMAT: ' + ZCustomer.FieldByName('alamat').AsString);
      ReportText.Add('MEMBER: ' + ZCustomer.FieldByName('member').AsString);
      ReportText.Add(''); // Add an empty line for separation
      ZCustomer.Next;
    end;
    ...
    // Show a message or perform any additional actions (e.g., open the file)
